    ...
    switch (action)
    {
        case 1:
            ...
            if(result)
            {
                ...
                player->GetSession()->SendNotification("同职业玩家：%s已第一个到达60级，无法领取奖励。", playerName.c_str());
                player->CLOSE_GOSSIP_MENU();
                break;
            }
...
void SendDefaultMenu_Foggy_Zoltan(Player *player, Creature *_Creature, uint32 action)
{
    switch (action)
    {
        case 1:
            player->CLOSE_GOSSIP_MENU();
            player->TeleportTo(MAP_EASTERN_KINGDOMS, -5618.981934f, -830.361633f, 483.534393f, 0.0f);
            break;
        case 2:
