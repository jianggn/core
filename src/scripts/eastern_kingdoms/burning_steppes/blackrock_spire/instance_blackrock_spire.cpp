    ...
    switch (pAt->id)
    {
        case AREATRIGGER_ENTER_UBRS:
            if (instance_blackrock_spire* pInstance = (instance_blackrock_spire*) pPlayer->GetInstanceData())
            {
                ...
                // Level 60 Hardcore Challenger With Item 12219 Can Enter UBRS
                if (pPlayer->HasItemCount(12344, 1) || (pPlayer->HasItemCount(12219, 1) && pPlayer->GetLevel() >= 60 && pPlayer->GetQuestStatus(10000) == QUEST_STATUS_COMPLETE)) // le joueur a le sceau de l'ascension
